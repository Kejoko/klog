...
int perform_checks(
    KlogString* const p_result,
    const uint32_t    length_provided,
    const char*       s_filename_provided,
    const uint32_t    line_provided
) {
    const uint32_t length_expected = length_provided + 1 + 4; /* colon, 4 digit line number */

    if (p_result->length != length_expected) {
        printf(
            "Resulting KlogString (\"%s\") length is actually %d instead of expected %d\n",
            p_result->s,
            p_result->length,
            length_expected
        );
    ...
    const uint32_t filename_character_copy_amount = filename_provided_length
        <= length_provided ? filename_provided_length : length_provided;
    ...
    if (filename_comparison_result) {
        printf(
            "strncmp result (%d) shows that filename portion was \"%.*s\" instead of \"%.*s\"\n",
            filename_comparison_result,
            length_provided,
            p_result->s,
            length_provided,
            s_adjusted_filename
    ...
    if (final_character != '\0') {
        printf(
            "Resulting KlogString's final character (index %d, reported length + 1) should be null terminator, but instead it is '%c'\n",
            length_expected,
            final_character
        );
        return 1;
