...
int do_comparison(
    ...
    for (uint32_t i_format_string = 0; i_format_string < expected_number_format_strings; ++i_format_string) {
        ...
        if (p_result->ls_strings[i_format_string] != expected_format_strings[i_format_string]) {
            printf(
                "[%s] format string %d address (%p) != expected (%p)\n",
                test_name,
                i_format_string,
                (void*)p_result->ls_strings[i_format_string],
                (void*)expected_format_strings[i_format_string]
            );
            return 1;
        }
        ...
        if (p_result->a_string_lengths[i_format_string] != expected_format_string_lengths[i_format_string]) {
            printf(
                "[%s] format string %d length (%d) != expected (%d)\n",
                test_name,
                i_format_string,
                p_result->a_string_lengths[i_format_string],
                expected_format_string_lengths[i_format_string]
            );
            return 1;
        }
        ...
        if (strcmp(p_result->ls_strings[i_format_string], expected_format_strings[i_format_string]) < 0) {
            printf(
                "[%s] format string %d (\"%.*s\") != expected (\"%.*s\")\n",
                test_name,
                i_format_string,
                expected_format_string_lengths[i_format_string],
                p_result->ls_strings[i_format_string],
                expected_format_string_lengths[i_format_string],
                expected_format_strings[i_format_string]
            );
            return 1;
        }
    }
