...
int just_thread_id(
    ...
    if (result.length != length_expected) {
        printf(
            "Resulting KlogString's length is %d but should be %d when only the thread id is provided\n",
            result.length,
            length_expected
        );
        return 1;
    ...
    if (comparison != 0) {
        printf(
            "strcmp result (%d) shows that resulting string was \"%.*s\" instead of \"%.*s\"\n",
            comparison,
            length_expected,
            result.s,
            length_expected,
...
int just_time(
    ...
    if (result.length != length_expected) {
        printf(
            "Resulting KlogString's length is %d but should be %d due to only the time string being provided length\n",
            result.length,
            length_expected
        );
        return 1;
...
int just_name(
    ...
    if (result.length != length_expected) {
        printf(
            "Resulting KlogString's length is %d but should be %d due to only the logger name string being provided\n",
            result.length,
            length_expected
        );
        return 1;
...
int just_level(
    ...
    if (result.length != length_expected) {
        printf(
            "Resulting KlogString's length is %d but should be %d due to only the level string being provided\n",
            result.length,
            length_expected
        );
        return 1;
...
int just_source_location(
    ...
    if (result.length != length_expected) {
        printf(
            "Resulting KlogString's length is %d but should be %d due to only the source location being provided\n",
            result.length,
            length_expected
        );
        return 1;
...
int nominal_parameters(
    ...
    if (comparison_thread_id != 0) {
        printf(
            "Thread id strncmp result (%d) shows that resulting string contains \"%.*s\" instead of \"%.*s\"\n",
    ...
    if (comparison_remainder != 0) {
        printf(
            "Remainder strcmp result (%d) shows that resulting string contains \"%s\" instead of \"%s\"\n",
            comparison_remainder,
            result.s + 29,
            s_expected + 29
        );
        return 1;
